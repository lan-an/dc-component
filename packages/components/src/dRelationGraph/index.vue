<template>
  <relation-graph ref="relationGraphRef" v-bind="{ ...$attrs }" :style="{width: !width ? '100%': `${width}px`,height: !height ? '100%': `${height}px`}">
...
defineExpose({
  ...
  focusNodeById: (nodeId: string)=> relationGraphRef.value.focusNodeById(nodeId),
  getNodeById: (nodeId: string)=> relationGraphRef.value.getNodeById(nodeId),
  removeNodeById: (nodeId: string)=> relationGraphRef.value.removeNodeById(nodeId),
